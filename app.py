...
if uploaded_file:
    try:
        ...
        # Garantir listas vazias mesmo se não houver dados
        if 'dados' not in locals(): dados = []
        if 'resumo' not in locals(): resumo = []
...
def gerar_resumo_cfop_cst_aliquota(df_itens):
    """
    Gera um resumo agrupado por CST, CFOP e Alíquota conforme o modelo fornecido.
    
    df_itens: DataFrame com colunas ['CST', 'CFOP', 'Aliquota', 'Valor', 'BaseICMS', 'ValorICMS']
    """
    resumo = (
        df_itens.groupby(['CST', 'CFOP', 'Aliquota'], dropna=False)
        .agg({
            'Valor': 'sum',
            'BaseICMS': 'sum',
            'ValorICMS': 'sum'
        ...
        .rename(columns={
            'Valor': 'Valor Total',
            'BaseICMS': 'Base de Calculo',
            'ValorICMS': 'V. ICMS'
        })
    )

    # Reorganizar colunas na ordem do modelo
    colunas_ordenadas = ['CST', 'CFOP', 'Aliquota', 'Valor Total', 'Base de Calculo', 'V. ICMS']
    resumo = resumo[colunas_ordenadas]

    return resumo

def salvar_planilha_com_resumo(df_itens, outras_abas=None, caminho_saida="saida.xlsx"):
    """
    Salva uma planilha Excel com múltiplas abas, incluindo o resumo por CFOP + CST + Alíquota.
    
    df_itens: DataFrame com os dados dos itens das notas.
    outras_abas: dict com {nome_aba: DataFrame} para incluir outras abas na planilha.
    caminho_saida: Caminho do arquivo Excel de saída.
    """
    resumo_df = gerar_resumo_cfop_cst_aliquota(df_itens)

    with pd.ExcelWriter(caminho_saida, engine='xlsxwriter') as writer:
        resumo_df.to_excel(writer, index=False, sheet_name="Resumo CFOP+CST+Aliq")
        if outras_abas:
            for nome, df in outras_abas.items():
                df.to_excel(writer, index=False, sheet_name=nome)



# Dados de exemplo adicionados manualmente
df_exemplo_resumo = pd.DataFrame([
    {"CST": "00", "CFOP": "5102", "Alíquota": "19", "Valor Total": 24.45, "Base de Cálculo": 24.45, "ICMS": 4.63},
    {"CST": "00", "CFOP": "5102", "Alíquota": "27", "Valor Total": 173.40, "Base de Cálculo": 173.40, "ICMS": 46.81},
    {"CST": "20", "CFOP": "5102", "Alíquota": "19", "Valor Total": 264.50, "Base de Cálculo": 97.39, "ICMS": 18.46},
    {"CST": "60", "CFOP": "5405", "Alíquota": "0", "Valor Total": 283.75, "Base de Cálculo": 0.00, "ICMS": 0.00},
])
escrever_aba(df_exemplo_resumo, "Resumo")
